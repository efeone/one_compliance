...
def prepare_data(departments, filters):
	data = []
	if filters.get("reference_type") in [None, "Project"]:
		for department in departments:
			...
			if filters.get("reference_type") in [None, "Event"] and not filters.get("project") and not filters.get("invoiced"):
				...
				if work:
					...
					events = frappe.db.get_all(
						"Event", filters=event_filters, fields=["*"]
					)
					for event in events:
						...
						row = {
							...
							"status": event["status"],
							"invoiced": "Yes",
							"invoiced_amount": event_billing_details["invoice_amount"],
							"payment_received": event_billing_details["payment_received"],
							"outstanding_amount": event_billing_details[
								"outstanding_amount"
							],
						}
						department_row.append(row)
	...
	if filters.get("reference_type") in [None, "Event"] and not filters.get("department") and not filters.get("project") and not filters.get("invoiced"):
...
def get_events_without_department(filters):
	event_rows = []
	valid_works = frappe.db.get_all("Compliance Sub Category", {"department":""}, pluck="name")
	...
	for event in events:
		event_billing_details = handle_event_billing(
			{"client": event["custom_customer"], "sub_category": event["custom_service"]}
		)
		row = {
			"invoiced_amount": 0,
			"payment_received": 0,
