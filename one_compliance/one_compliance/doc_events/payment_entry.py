	...
	for payment_reference in doc.references:
		if payment_reference.reference_doctype == "Sales Invoice":
			...
			if payment_reference.outstanding_amount == 0:
				...
				frappe.db.set_value("Project", frappe.db.get_value("Sales Order", sales_order, "project"), "status", "Paid")
			elif (
				...
				frappe.db.set_value("Project", frappe.db.get_value("Sales Order", sales_order, "project"), "status", "Partially Paid")
