frappe.ui.form.on('Sales Order', {
	...
	refresh: function (frm) {
		if (
			!frm.is_new() &&
			frm.doc.docstatus == 1 &&
			!frm.doc.custom_create_project_automatically
		) {
		...
		fields: [
			...
			},
			{
				...
				onchange: function () {
					...
					frappe.call({
						method:
							'one_compliance.one_compliance.doc_events.sales_order.get_compliance_subcategory',
						args: {
							item_code: item_code,
						},
						callback: function (r) {
							if (r.message) {
								...
								d.set_value(
									'compliance_category',
									compliance_sub_category.compliance_category
								);
								d.set_value(
									'compliance_sub_category',
									compliance_sub_category.name
								);
								d.set_value(
									'project_template',
									compliance_sub_category.project_template
								);
							}
						},
					});
		...
		primary_action(values) {
			frm.doc.items.forEach((item) => {
				if (item.item_code == values.item) {
					frappe.call({
						method:
							'one_compliance.one_compliance.doc_events.sales_order.create_project_from_sales_order',
						args: {
		...
		if (child_doc.item_code) {
			frappe.call({
				...
				callback: function (response) {
					if (response.message) {
						child_doc.custom_compliance_category =
							response.message.compliance_category;
...
frappe.ui.form.on('Reimbursement Details', {
	approve: function (frm, cdt, cdn) {
		...
		frappe.call({
			method:
				'one_compliance.one_compliance.doc_events.sales_order.submit_journal_entry',
			args: {
...
function create_purchase_invoice(frm) {
	...
	frappe.prompt(
		...
		(values) => {
			frappe.call({
				method:
					'one_compliance.one_compliance.doc_events.sales_order.create_purchase_invoice',
				args: {
				...
				callback: function (r) {
					if (r.message) {
						...
						frm.reload_doc();
						frappe.msgprint(
							__('Purchase Invoice {0} created successfully', [r.message])
						);
					}
				},
			});
...
/**
 * Add Purchase Invoice button if conditions are met
 */
function add_pi_button(frm) {
	// Show button only if checkbox is checked and no PI exists yet
	if (
		!frm.is_new() &&
		!frm.is_dirty() &&
		frm.doc.docstatus === 1 &&
		frm.doc.is_outsource_service &&
